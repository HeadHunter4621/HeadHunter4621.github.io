...
DeepVocal uses an aliasing system called CVVX (this is what I used when I made [UTAU CVVX](/resources/cvvx-phonemizer/), lol). It is similar to UTAU CVVC, with some slight exceptions (Independent symbols, VC and VV phonemes being treated the same).

## Choosing a Reclist:
For DeepVocal, you should use either a CVVC reclist. While a VCV reclist would work, there is no point in wasting time with it, since DV has no support for triphones. I have personally made a [DeepVocal-specific reclist](), however it is currently still in development. Additionally, DeepVocal does not accept `- c` phonemes (from my testing), so in order to get an acurrate sound you'll need to have `-cv` phonemes (these are treated as normal CV, for some reason). I recommend [Salem Wasteland's JA CVVC 2-Mora reclist](https://utau.felinewasteland.com/jp/cvvc#2) (standard) because it has all phonemes that DeepVocal will use. Also, VV transitions aren't technically necessary. 

While CV DeepVocal is possible, I do not recommend it because it sounds bad (lol).

## Recording with Recstar

Since I'll assume you're using the above-linked reclist, this will be specific to that. For other reclists, you'll have to do your own research. This section is more spotty than the other sections since I don't especiall want to spend my tie describing a process that is often used for UTAU recording as well.
...
1. By default, when you press and hold the space bar, RecStar will start recording. Generally this is fine, and it's the method that I use.
